...
if not st.session_state.get("authenticated") or st.session_state.get("username") != "admin":
    st.error("Acesso restrito à administração."); st.page_link("main.py", label="Voltar para o Início"); st.stop()
        ...
        if st.button("Salvar Alterações", type="primary"):
            for i, original_row in todos_usuarios.iterrows():
                ...
                if edited_row["deletar"]:
                    auth.delete_user(original_username); st.toast(f"Usuário '{original_username}' deletado."); continue

                if original_username != edited_row["username"]:
                    success, message = auth.update_username(original_username, edited_row["username"])
                    if success:
                        st.toast(message); original_username = edited_row["username"]
                    else:
                        st.error(message)

                if edited_row["nova_senha"]:
                    auth.update_user_password(original_username, edited_row["nova_senha"]); st.toast(f"Senha de '{original_username}' atualizada.")
            
            st.success("Alterações processadas! Recarregando a lista..."); st.rerun()
